...
import { clinicalTabs } from "./clinicalDataData";

const ClinicalData = ({ onComplete }) => {

  const [activeTab, setActiveTab] =
    useState(clinicalTabs[0].id);
  ...
  const currentTab = clinicalTabs.find(
  ...
  const handleSelect = (option) => {
    ...
    if (!isLastQuestion) {
      ...
      const isLastTab =
        currentTabIndex ===
        clinicalTabs.length - 1;

      if (!isLastTab) {

        setActiveTab(
          clinicalTabs[currentTabIndex + 1].id
        );
  ...
  return (
    <CategoryCard title="Clinical Data">

      <QuestionTabs
        tabs={clinicalTabs.map(
          (tab) => tab.title
        )}
